...
const ShopifyProduct = () => {
  ...
  const getShopifyProduct = () => {
    axios.get("/ShopifyProduct/" + storeName.toLowerCase()).then((data) => {
      console.log("data daat products", data.data.products);
      let checkStoreShopify = [];
      data.data.products.forEach((item, i) => {
        if (
          data.data.products[i].vendor.toLowerCase() ===
          decode.store.toLowerCase()
        ) {
  ...
  const UpdateProduct = async (e) => {
    ...
    await axios
      .put("/ShopifyProduct/" + storeName.toLowerCase() + "/" + code, product)
      .then((data) => {
        console.log(data);
        if (data) {
          getShopifyProduct();
          NotificationManager.success(
            "Product Updated in Shopify Successfully"
          );
  ...
  const deleteProduct = (data) => {
    axios
      .delete("/ShopifyProduct/" + storeName.toLowerCase() + "/" + data.id)
      .then((data) => {
        if (data) {
          getShopifyProduct();
          NotificationManager.success(
            "product Deleted  from Shopify Successfully"
          );
        } else {
          NotificationManager.error("Something wrong");
        }
